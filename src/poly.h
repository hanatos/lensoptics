...
float poly_pow(const float x, const int32_t exp)
{
...
float poly_evaluate(const poly_t *poly, const float *x, int max_degree)
{
  float out = 0.0f;
  ...
  for(int j=0;j<poly_num_vars;j++)
  {
    for(int t=0;t<s->poly[j].num_terms;t++)
    {
      if(fabsf(s->poly[j].term[t].coeff) < poly_coeff_eps)
      {
        ...
        {
          memcpy(s->poly[j].term+t, s->poly[j].term+s->poly[j].num_terms-1, sizeof(poly_term_t));
          t--;
          s->poly[j].num_terms--;
        }
      }
